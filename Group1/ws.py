...
import os
import re
from ev3dev2.motor import MoveJoystick, list_motors, LargeMotor, MoveTank, SpeedPercent, OUTPUT_C, OUTPUT_B
...
class TankWebHandler(RobotWebHandler):
    ...
    def do_GET(self):
        ...
        if action == "sound":
            if "%20" in str(path[3]):
                path[3] = path[3].replace("%20", " ")
            log.debug(path[3])
            sound = Sound()
            sound.speak(path[3])
        elif action == "rotate":
            # log.debug(int(path[3]))
            ...
            # tank_drive = MoveTank(self.robot.left_motor, self.robot.right_motor)
            # tank_drive.on_for_rotations(left_speed, right_speed, int(path[3]))
            tank_drive = MoveTank(OUTPUT_B, OUTPUT_C)
            tank_drive.on_for_rotations(SpeedPercent(50), SpeedPercent(50), int(path[3]))
